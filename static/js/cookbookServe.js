...
function _toContainerPath(p) {
  ...
  if (/^[a-zA-Z]:[\/\\]/.test(p.replace(/\//g, '\\'))) {
    ...
    if (idx !== -1) {
      const rest = normP2.slice(idx + '/huggingface/hub/'.length);
      return `${containerHub}/${rest}`;
    }
  }
  return p;
}
...
function _ggufSearchDirExpr(model, repo) {
  ...
  const serveDir = _toContainerPath(searchDir.startsWith('~') ? searchDir.replace(/^~/, '${HOME}') : searchDir);
  return _shellQuote(serveDir);
}
